...
The Evals view contains the ten-case evaluation runner. The first five cases preserve the official capstone baseline; EVAL-06 and EVAL-07 add the original PRD billing-escalation and silent-no-action cases; EVAL-08 and EVAL-09 add supported subscribe and duration-aware pause decisions; and EVAL-10 tests the child-rating boundary and title-specific adult exception. Pause recommendations carry three independently validated structured values—selected calendar days, maximum permitted days, and avoided billing cycles—and must state the selected duration in days and explicitly identify the renewal date, so models cannot conflate a 57-day pause with two avoided monthly charges or silently treat the renewal as an unlabeled start date. The semantic judge accepts natural equivalents such as keeping or retaining an existing record instead of requiring the literal phrase “no record change.” A compact results dashboard continuously shows passed, failed, error, and not-run totals. The top action bar keeps **Run all cases**, **Copy output**, and **Clear results** visible, while **Rejudge saved outputs** and **Revoke instructions approval** live under **More**. During an active run, **Stop tests** replaces **Run all cases**, aborts the in-flight model request, preserves completed results, and leaves unstarted cases as not run. The separate **Instructions** button in the page header opens the six exact instruction files, fingerprint, and approval gate without reducing the result area. The results-first workspace uses a narrow selectable ten-case list and gives the remaining space to one focused case. Human-readable input and output appear first; the case definition, deterministic and judge checks, structured response, and raw judge result are collapsible. On narrow screens, the case list becomes a horizontal selector above the result. No evaluation is available until the user reviews the instruction bundle and explicitly approves its current fingerprint. That approval is tied only to the instruction bundle: switching agent or judge providers and models preserves it, while editing any instruction automatically invalidates it. Model or case changes can still make prior results noncurrent without requiring the unchanged instructions to be approved again. Nine cases each make one call to the selected agent model and one call to the separately selected semantic-judge model; EVAL-07 runs its fixed inputs through the shared signal detector locally and verifies that the no-action path makes no model call. Judge evidence for a passed requirement is schema-constrained to one complete adult-facing response field with line breaks normalized to spaces, preventing the judge from inventing or paraphrasing a purported exact quote while keeping multiline conversation replies compatible with strict structured output. Any other structurally invalid judge response is retried once with the exact validation failure while preserving the rejected judge output for review. **Copy output** creates a paste-ready report containing every saved verdict, structured check, judge assessment, rejected judge attempt, error, prompt hash, and complete output. **Rejudge saved outputs** sends compatible saved agent outputs—including valid outputs retained after a judge-only error—to the selected judge without regenerating the responses; compatibility requires the same agent model, agent instructions, and case definitions.
...
│   │   ├── verify-provider-connectors.mjs
...
The repeatable context-search benchmark contains 127 labeled keyword, semantic, mixed, negative, held-out, and portfolio-comparison queries. The current implementation passes 127 of 127 cases (100%), exceeding the 95% target. Broad comparative savings requests retrieve the active subscription portfolio and its relevant plan, contract, watchlist, viewing, and budget records; entity-specific questions remain focused, and unrelated savings questions retrieve no household context. Run the benchmark with `node tests/scripts/context-search-benchmark.mjs --require=95`; the machine-readable result is saved to `tests/reports/context_search_benchmark_results.json`.

Run `node tests/scripts/run-quality-gates.mjs` for the complete deterministic release gate. It verifies project structure and publication boundaries; executes the dedicated feedback and regression-capture workflow suite; runs 102 focused component assertions across memory, financial math, safety and validation, response completeness, workflow, provider configuration, and UI contracts; exercises seven keyless mocked Anthropic and Gemini connector contracts; runs the 127-case context benchmark; and completes the cross-component regression. All deterministic gates must pass at 100%. Machine-readable results are written to `tests/reports/project_structure_results.json`, `tests/reports/feedback_regression_loop_results.json`, `tests/reports/component_quality_results.json`, `tests/reports/provider_connector_results.json`, `tests/reports/context_search_benchmark_results.json`, and `tests/reports/deterministic_quality_summary.json`.
...
The evaluation runner does not use keyword matching to assign semantic pass/fail verdicts; clear paraphrases, natural evidence statements, and synonyms are assessed for meaning by the judge. JavaScript checks structured target and action fields directly, extracts exact URLs, complete calendar dates, and currency amounts for source-backed grounding validation, and blocks a small set of explicit unsafe or contradictory response forms. Those narrow guards distinguish positive recommendations from language that explicitly withholds or defers a recommendation while adult judgment is required. When deterministic checks pass, the judge treats each explicitly validated property as authoritative instead of second-guessing it from public brand names. A material fact can satisfy the rubric wherever it appears in the complete response; technical record labels and repetition in a dedicated evidence array are not required. The judge still independently checks that every materially required fact and behavior is present. Every case also includes a manual-review panel with a plain-English version of the complete fixed input and the actual output. Raw structured agent and judge results—or the complete local workflow result for EVAL-07—remain available underneath for technical inspection, and the copy-all export includes both readable and structured evidence.
