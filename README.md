...
Live-model adult messages remain transient until the structured safety disposition is validated. In-scope messages then enter browser-local chat history; sensitive and out-of-scope messages are stored only as neutral redaction notices. Obvious credentials and payment details are intercepted locally before a model request, recognizable sensitive content in previously saved chat is redacted when state loads, and a message that cannot be safely classified because the model call fails is discarded. Safety-only turns cannot propose or apply household-context updates.
...
No tool can modify a streaming account or send a message through an external platform. Tool activity is recorded in local household memory for inspection.
