...
In the prototype, other family members are represented in household context through their watchlists, viewing confirmations, priorities, ages, and rating limits, but they do not have individual chats, authenticated user identities, or separate permissioned accounts. Individual family-member experiences and permission levels are reserved for a production implementation.
...
The release standard requires 100% passing results for deterministic component, safety, workflow, financial, and context-retrieval tests. Because model responses and semantic judging are probabilistic, model-driven evaluation cases must achieve at least a 95% pass rate across repeated runs. The latest official ten-case run achieved 10 of 10 passes.
...
A third Enter a manual scenario path demonstrates the agent’s broader conversational and safety boundaries. The adult may enter any nonsensitive free-text message. The selected LLM interprets the message and determines whether it relates to streaming-subscription planning, management, viewing access, or spending; deterministic keyword or exact-match logic does not decide the meaning of eligible free text. The adult can ask a subscription-planning question, report a completed subscription or household-information change, provide recommendation feedback, or request an external action. If the request is outside scope, the agent politely explains its boundary and invites a relevant question without updating household information. If asked to subscribe to Summit+ directly, Streaming Guard presents a structured refusal explaining that it cannot access provider accounts, make payments, or complete subscription changes. It directs the adult to complete the action through the streaming service and confirm afterward; the refusal itself does not change the household’s subscription record.
...
The agent reads the current date; the household’s fictional prototype profile, watchlists, manually reported viewing status, priorities, preferences, active subscriptions, prices, billing dates, monthly budget, and family content rules; and the relevant recent conversation. It also reads fictional service-plan and catalog records covering releases, availability changes, migrations, promotions, bundles, pause limits, cancellation terms, and account URLs. Before calling the LLM, deterministic hybrid retrieval analyzes the current trigger or adult message, recent conversation, active recommendation, and recognized services, plans, titles, and household members. It combines exact keyword matching, typo-tolerant fuzzy matching, local semantic concepts, and household relationships to select relevant subscription, watchlist, viewing, catalog, service-term, financial, and policy records. The application records a technical ContextPlan for retrieval validation and the reviewer trace, but does not send that technical plan, filenames, source metadata, record identifiers, or selection explanations to the LLM. Instead, it transforms the selected records into an audience-safe household-information bundle with plain-language labels. Broad portfolio questions receive broader household information, while narrow questions receive a smaller relevant packet. Every nonsensitive free-text message is sent to the selected LLM for interpretation; deterministic retrieval may restrict unrelated household information, but it does not decide the message’s meaning or produce a local semantic response.
...
When one action is clearly supported, the agent produces a structured subscription recommendation in chat containing the proposed action, affected service, triggering event, financial impact, relevant titles, household-fit rationale, supporting facts in plain English, uncertainty, and the specific adult response or external action required. When a recommendation is not yet supportable, it asks for the exact missing information or adult judgment needed. It may also answer a subscription-related question, politely redirect an out-of-scope request, present a structured refusal or safety escalation, interpret recommendation feedback, propose a lasting household preference for adult review, or propose a validated household-information update. Adult-visible responses use ordinary household language rather than filenames, schemas, prompts, retrieval terminology, internal record identifiers, or implementation details. Any account URL is taken from `streaming_services.csv` and provided only for manual adult use.

#### Check

Before returning the result, the prototype verifies that required information is present, current, and internally consistent; validates structured output and deterministic date and financial calculations; grounds title, price, availability, billing, service-term, and household claims in the supplied records; applies child-rating and family rules; and validates any included URL against `streaming_services.csv`. It also verifies that the selected household information covers the entities and records needed for the request. Adult-visible response fields are checked for internal implementation language such as filenames, schemas, prompts, tool outputs, context-selection terminology, hashes, raw record identifiers, and JSON instructions. If this validation fails, the application automatically asks the model once to rewrite the response without changing its supported meaning. If URL validation fails, the link is omitted and the adult is told that no verified link is available. If a viewing request would exceed the household budget, the agent explains the amount and asks the adult whether to reprioritize, choose a cheaper option, or explicitly change the budget. A recommendation-driven subscription record does not change until the adult explicitly confirms completing the required external action. Through free-text chat, independently completed subscription or plan changes may also be validated and saved as explicit new household information. Retrieval and validation code do not select the final recommendation; the selected LLM still makes that judgment.
...
- **selected_household_information [dynamic input]:** Before each LLM call, the prototype creates and validates an internal request-specific ContextPlan. Hybrid retrieval combines exact keyword matching, typo-tolerant fuzzy matching, local semantic concepts, and household relationships among services, plans, titles, viewers, subscriptions, watchlists, viewing records, releases, ratings, rules, and calculations. The internal ContextPlan records inferred intent and scope, resolved entities, required record types, selected-record counts, selection reasons, coverage status, information budget, and a stable trace hash. It is used for application validation and the evaluator trace, not as the household-information payload sent to the LLM. The selected records are transformed into an audience-safe, plain-language household-information bundle that omits filenames, technical selection metadata, hashes, schemas, and raw internal identifiers. Broad inventory, comparison, or planning questions expand to relevant portfolio-level information. Potentially unrelated nonsensitive messages still reach the LLM for scope judgment without exposing unrelated household data.
...
- **conversation_add_on.md [conversation instruction]:** Defines how the agent answers questions, requests clarification, handles recommendation discussions, records explicit household-information updates, confirms external actions, refuses unsupported execution requests, interprets recommendation feedback, distinguishes one-time feedback from a possible lasting household preference, proposes the smallest relevant preference for adult review, and avoids saving a lasting preference without explicit adult approval.

`build/build-knowledge.mjs` compiles the source data, policies, and instructions into `js/knowledge-base.js` for browser use. Household changes are validated and persisted locally through the prototype’s structured memory layer and browser localStorage.
...
- **tests/fixtures/agent_evals.csv:** Defines the broader collection of scenario, boundary, and data-quality cases available to the prototype.
- **tests/fixtures/eval_cases.csv:** Defines the ten cases in the current scored evaluation set, including their fixed inputs and expected behavior.
- **tests/instructions/evaluation_judge.md:** Defines the independent semantic judge’s assessment criteria without relying on exact-word matching.

Final evaluation evidence is stored separately in `tests/results/final_evaluation_results.md` and `tests/results/final_evaluation_summary.md`. Machine-readable deterministic and context-test reports are stored in `tests/reports/`.
...
- **select_context [implemented deterministic hybrid retrieval tool]:** Creates and validates a request-specific ContextPlan using exact keyword matching, typo-tolerant fuzzy matching, local semantic concepts, and household relationships. It resolves named and implied services, plans, titles, viewers, subscription records, watchlist and viewing records, family rules, calculations, and recent conversation state. It expands broad inventory, comparison, and planning requests to portfolio-level information, records coverage and selection reasons, and withholds unrelated household information when the request does not justify retrieving it. This retrieval boundary does not determine the meaning or scope of an eligible free-text message; every nonsensitive free-text message still reaches the selected LLM for interpretation. The tool does not choose the recommendation.
...
- **update_household_context [implemented controlled write tool]:** Validates and saves explicit information supplied or explicitly approved by the authorized adult to the prototype’s local household information. Supported changes include subscription additions, cancellations, pauses, reactivations, plan and price changes, renewal settings and dates, expiration dates, viewing confirmations, watchlist status and priority, budget and household preferences, adult-approved lasting preferences, family rules, household-added escalations, and one-time title-specific child-rating exceptions. The tool records the source and update time and never modifies an external provider account or parental-control system. Recommendation feedback, rejected preference proposals, and unapproved preference drafts remain separate and cannot modify durable household information.
...
- **send_chat_response [implemented local-output tool]:** Renders structured text, recommendations, blocking choices, confirmations, four-section refusals, feedback acknowledgments, preference-review prompts, and safety responses in the active WhatsApp-style chat interface. Every adult message remains visible in the current chat exactly as entered. Recognizable sensitive information is intercepted locally before any model request and is excluded from persistent chat history, household information, AI request logs, and model requests. Every nonsensitive free-text message, including a message that may ultimately be outside scope, is sent to the selected LLM for interpretation and scope judgment. If the model determines that a request is outside scope, the message remains visible for the current session, cannot update household information, and is not retained as durable household or chat memory. The tool does not connect to WhatsApp or another external messaging platform.

#### Supporting prototype systems

- **Multi-provider model interface [implemented external API interface]:** Sends the same global instructions, applicable task add-on, audience-safe household-information bundle, relevant recent conversation, and structured-output contract to the selected OpenAI, Anthropic, or Google model. Technical ContextPlan metadata, filenames, hashes, schemas, source-selection explanations, and raw record identifiers remain in the application’s reviewer trace and are not sent as household information. Recommendation generation and independent evaluation judging can use separately selected models.
- **Browser-local structured persistence [implemented local persistence]:** Stores durable household state and transient session state as separate versioned documents with schema validation and legacy-state migrations. A persistence adapter and revision-checked repository boundary isolate localStorage from application logic. Evaluation results and model settings are stored separately. The state survives refreshes on the same browser but is not a production database and does not synchronize across devices.
...
- **Recommendation feedback workflow [implemented feedback system]:** Lets the adult rate a recommendation as helpful or poor, select feedback reasons, and provide free-text comments. The selected LLM determines whether the feedback is one-time commentary or may imply a lasting household preference. A possible lasting preference is presented through blocking choices: Save preference, Don’t save, Edit, or Ask a question. Save and rejection are controlled application actions; Edit and Ask a question return to the LLM. No lasting preference enters durable household memory without explicit adult approval.
- **Regression-draft capture [implemented reviewer workflow]:** Creates a draft regression case when an adult identifies a poor recommendation or when an evaluation fails or errors. Drafts preserve the relevant input, output, failure reason, and instruction fingerprint for reviewer analysis. They do not automatically modify the official evaluation suite; a human must review and promote a draft before it becomes an active regression case.
- **Live context, policy, and execution trace [implemented evaluator interface]:** Shows which relevant data sources, instructions and policies, simulated tools, validation steps, and memory outcomes were used during each guided recommendation or free-text interaction. Each interaction creates a privacy-safe trace linking the adult input, instruction fingerprint, ContextPlan hash, selected provider and model, model response, validation result, simulated tools, workflow transitions, and any controlled memory command. The trace updates as the interaction progresses and distinguishes an unchanged household record from an actual saved memory change. It is visible for prototype evaluation and is not part of the adult-facing WhatsApp conversation or exported durable household data.

### 6. Memory decision

*What should the agent remember, and what should it not remember?*

The prototype uses limited, structured browser-local persistence across runs. The source data files provide the initial fictional household state, while validated adult-confirmed changes are persisted in browser localStorage. The application separates durable household memory from transient session state using independently versioned documents, schema validation, and legacy-state migrations. This allows state to survive refreshes and browser restarts on the same device, but it does not synchronize across browsers, users, or devices and is not a production database.
Durable household memory includes the household profile; family members and preferences; current subscriptions and subscription-change history; spending history and recommendation-savings events; per-member watchlists, priorities, and acceptable waiting periods; manually reported viewing status and completion dates; the monthly budget and household preferences; family content-rating limits; one-time title-specific child-rating exceptions; household-added escalation conditions; record provenance; the current household revision; and applied command IDs used to prevent duplicate writes.
Session and supporting application state are stored separately from durable household memory. They include the active scenario, current recommendation and discussion state, eligible browser-local chat history, reminders, workflow history, simulated-tool audit, execution traces, recommendation-feedback records, pending preference proposals, regression-case drafts, evaluation results, and model connection settings. Adult-approved lasting household preferences are stored in durable household memory and appear in the Context tab; rejected or unapproved preference proposals do not affect future recommendations. Recommendation feedback and regression drafts remain separate from household memory. Provider API keys are never included in household information sent to the model or in household-data exports.
The Context tab allows the adult to export durable household data as JSON and import a compatible household-data file. Exports exclude chat, recommendation progress, reminders, traces, tool activity, evaluation results, and provider keys. An import is validated and migrated when necessary, then starts a fresh session around the imported household state.
...
Every adult message remains visible in the current chat exactly as entered. Recognizable credentials, payment information, authentication codes, and other sensitive information are intercepted locally before any model request. They remain visible only for the current session and are excluded from persistent chat history, household information, AI request logs, and model requests; the interface does not replace or mask the visible adult message. Every nonsensitive free-text message is sent to the selected LLM for interpretation, including a message that may ultimately be classified as outside scope. If the model determines that a message is outside scope, it politely redirects the adult, does not update household information, and does not retain the message as durable chat or household memory. Recognizable sensitive information found in older saved state is removed when that state loads. A model-call or classification failure does not replace or mask the adult’s visible message, and safety-only or out-of-scope interactions cannot update household information.

### 7. Output format

*What should the agent produce so a human can review it quickly and confidently?*

For model-driven recommendations and conversations, the LLM returns strict structured JSON that follows the applicable recommendation or conversation schema. The browser validates the structure, grounding, allowed action, and adult-visible language before rendering it as a human-readable response in the WhatsApp-style chat interface. Adult-visible fields may not expose filenames, schemas, prompts, tool outputs, context-selection terminology, hashes, raw record identifiers, or JSON instructions. If otherwise valid output contains this internal language, the application automatically requests one faithful rewrite from the model. Technical request and response details remain available only to the prototype reviewer through the explicit Copy AI log debugging export and evaluator trace. The local no-action workflow does not call an LLM. The prototype does not generate or send email.
...
The response contract separately records whether the discussion remains open, is resolved, or is waiting for an external action; what input is needed next; whether the recommendation should remain, change, close, or reopen; and whether any structured household-information update or lasting-preference proposal is pending.
...
#### Recommendation feedback and preference review

When the adult submits recommendation feedback, the interface confirms that the feedback was saved separately from household memory. If the selected LLM identifies a possible lasting household preference, the agent presents the smallest proposed preference in plain language and displays the blocking choices Save preference, Don’t save, Edit, and Ask a question. The proposal is not treated as saved household information until the adult explicitly selects Save preference. Edit and Ask a question initiate another LLM interaction; they do not save the proposal.
...
The agent applies a child’s stored movie or television rating limit only when the title is on that child’s watchlist or the child is explicitly identified as an intended viewer. It does not apply a child’s rating restriction to a title merely because the household includes someone under age 18, and it does not flag an adult-only title when no child has requested it or is an intended viewer. If an applicable title exceeds the child’s limit, the agent does not recommend subscribing, keeping, or otherwise paying for access based on that title. It identifies the title, rating, affected child, age, and applicable limit, then asks the authorized adult whether they approve an exception for that specific title and child.
...
The agent never sends, stores persistently, or uses passwords, payment-card details, bank information, authentication codes, API keys, or other account credentials. Recognizable sensitive information is intercepted locally before a model request. The adult’s original message remains visible exactly as entered for the current session, but it is excluded from persistent chat history, household information, AI request logs, and model requests. Recognizable sensitive information found in older saved state is removed when that state loads. The agent warns the adult not to share sensitive information and directs them to complete sensitive account activity through the streaming service’s official interface.

#### Out-of-scope requests

Every nonsensitive free-text message is sent to the selected LLM for interpretation and scope judgment. If the model determines that the request is unrelated to household streaming-subscription planning, management, viewing access, or spending, it does not answer or perform the request. It politely explains its scope and invites a relevant question. The adult’s original message remains visible exactly as entered for the current session. The request cannot update household information or enter durable chat or household memory. Recognizable sensitive information remains the only category intercepted locally before a model request.
...
Recommendation feedback creates a separate human approval point. After the adult submits feedback, the selected LLM determines whether it is one-time feedback or may imply a lasting household preference. One-time feedback is recorded separately and does not change household memory. A possible lasting preference is presented through blocking choices: Save preference, Don’t save, Edit, or Ask a question. Save preference and Don’t save are controlled application decisions. Edit and Ask a question trigger another LLM interaction. The proposed preference affects future recommendations only after the adult explicitly selects Save preference. Poor recommendations and evaluation failures may also create draft regression cases, but only a reviewer may promote a draft into the official evaluation suite.
...
A separate automated release gate tests deterministic application components independently of the live LLM evaluations. It currently contains 93 component assertions covering memory, controlled writes, persistence, financial calculations, safety and validation, workflow behavior, provider configuration, audience-language boundaries, and UI contracts. A dedicated feedback and regression-capture suite contains 21 checks. A 126-case hybrid-retrieval benchmark covers keyword, semantic, mixed, negative, held-out, and portfolio-comparison queries. Cross-component regression also verifies all ten official evaluation information packets and 40 positive retrieval fixtures. Deterministic gates must pass at 100%; repeated model-driven evaluations must achieve at least a 95% pass rate.
...
- **Context:** The prototype creates and validates a request-specific ContextPlan using hybrid exact-keyword, typo-tolerant fuzzy, local semantic-concept, and household-relationship matching. It selects the relevant household, subscription, title, service, viewing, financial, and policy context and expands broad portfolio questions when necessary.
...
- **Enter a manual scenario:** Lets the adult enter any nonsensitive free-text message for LLM interpretation and scope judgment. The adult may ask a subscription-planning question, report a new subscription, cancellation, pause, plan change, budget change, watchlist update, or viewing confirmation, provide recommendation feedback, propose a lasting preference, request an external action, or test an out-of-scope boundary.
...
The Context and Spending tabs show stored household details and financial history. The Context tab also provides Export household data and Import household data controls. Export downloads only durable household memory as JSON, excluding chat, traces, recommendation progress, evaluation results, and provider keys. Import validates and migrates a compatible household document and then starts a fresh session around the imported household. The Evals tab runs and reviews the ten-case evaluation suite.
...
- **`tests/fixtures/agent_evals.csv` — 13:** Agent scenarios, triggers, targets, requested actions, and expected routes.
- **`tests/fixtures/eval_cases.csv` — 10:** The ten active fixed evaluation definitions.

`policies/family_rules.md` supplies the household’s configurable budget, viewing, and content preferences, while `data/family_rules.json` supplies the structured runtime version of those rules.
Five live-agent instruction files under `instructions/` define the global system behavior, immutable escalation rules, recommendation behavior, conversational behavior, and runtime grounding rules. The sixth instruction file, `tests/instructions/evaluation_judge.md`, defines the independent evaluation judge and is used only by the evaluation runner.
...
Testing is divided into deterministic and probabilistic layers. The deterministic release gate requires 100% passing results and contains 264 explicitly counted tests: 102 component-quality checks, 127 context-retrieval benchmark cases, 21 feedback and regression-workflow checks, 7 provider-connector contract checks, and 7 structure and publishing-boundary checks. One separately reported compound cross-component gate covers all ten official evaluation information packets, 40 positive retrieval fixtures, and additional workflow and integration assertions. The live evaluation layer tests the selected agent and judge models against the ten fixed cases and requires at least a 95% pass rate across repeated runs.
...
The deterministic release gate passed all 264 explicitly counted tests: 102 of 102 component-quality checks, 127 of 127 context-retrieval benchmark cases, 21 of 21 feedback and regression-workflow checks, 7 of 7 provider-connector contract checks, and 7 of 7 structure and publishing-boundary checks. The separately reported compound cross-component gate also passed, covering all ten official evaluation information packets, all 40 positive retrieval fixtures, and additional workflow and integration assertions. The generated household-information bundles also passed the audience-language checks that prevent filenames, technical selection metadata, schemas, hashes, and raw internal identifiers from being presented as household information to the LLM.
The official final live evaluation run was completed on August 8, 2026 at 11:44 AM PDT. Its provider-independent instruction-bundle fingerprint is `a96272d6`, as displayed in the UI, and its exported evaluation-configuration fingerprint is `966b686e`, covering the selected models, instruction bundle, and ten case definitions. Nine model-driven cases used `gpt-5.6-terra` for agent responses and `gpt-5.6-luna` for independent semantic judging. EVAL-07 used the shared deterministic signal detector and made no model call. All ten cases passed with zero validation failures, zero API errors, zero material judge gaps, and preserved human-control and safety boundaries. This run was completed after the feedback workflow, nonsensitive free-text routing, lasting-preference approval, regression-draft capture, hybrid-retrieval, and audience-language changes, so it is the official final evaluation evidence for the submitted prototype.
...
1. Added child-safety enforcement that applies a child’s stored movie or television rating limit only when the title is on that child’s watchlist or the child is explicitly identified as an intended viewer; the prototype does not flag unrelated titles merely because the household includes members under age 18.
...
1. Added a structured conversation contract covering turn type, discussion status, outcome, final action, external-action requirement, next expected input, safety disposition, reason codes, preference disposition, and proposed household-information updates.
...
1. Added a safety-gated chat workflow that keeps every adult message visible exactly as entered. Recognizable sensitive information is intercepted before a model request and excluded from persistent chat history, household information, AI request logs, and model requests. Every nonsensitive free-text message reaches the selected LLM for interpretation and scope judgment. A model-classified out-of-scope turn remains visible for the current session but cannot update household information or enter durable chat or household memory. Recognizable credentials in older saved state are removed during migration.
1. Added validated hybrid retrieval combining exact keywords, typo-tolerant fuzzy matching, local semantic concepts, and household relationships. Before each LLM call, the prototype creates an internal structured ContextPlan containing the inferred intent and scope, resolved services, plans, titles and viewers, required record types, selected-record counts, coverage status, information budget, selection reasons, and a stable trace hash. The application then transforms the selected records into an audience-safe household-information bundle. Technical ContextPlan metadata, filenames, schemas, hashes, and raw record identifiers remain available only to application validation and the reviewer trace.
1. Added broad-query expansion so inventory, comparison, and open-ended planning questions receive complete relevant portfolio context, while domain gating prevents unrelated requests from retrieving household data.
...
1. Added privacy-safe end-to-end execution traces that link each interaction’s input, instruction fingerprint, ContextPlan hash, model response, validation result, simulated tools, workflow transitions, and any persistent memory write. Traces remain session data and are excluded from exported durable household data.
1. Added an adult-visible language validator that rejects filenames, schemas, prompts, tool outputs, context-selection terminology, hashes, raw record identifiers, and JSON instructions in rendered model responses and requests one automatic faithful rewrite.
1. Added a recommendation-feedback workflow that separates one-time feedback from possible lasting household preferences and requires explicit adult approval before saving a lasting preference.
1. Added blocking Save preference, Don’t save, Edit, and Ask a question choices; Edit and Ask a question trigger another LLM call, while saving and rejection remain controlled application actions.
1. Added automatic draft regression-case capture for poor recommendations and evaluation failures. Drafts remain reviewer-controlled and do not enter the official evaluation suite without human promotion.
1. Removed local semantic responses and local out-of-scope interpretation for eligible free text. Every nonsensitive free-text message now reaches the selected LLM, while deterministic code remains responsible for privacy screening, retrieval, validation, calculations, controlled writes, and unavailable-model messaging.

One genuine agent-side failure occurred in EVAL-09. The model selected the correct Pause action but framed a 57-day viewing gap as a two-month pause, conflating the calendar duration with the two avoided monthly billing cycles. I corrected the runtime contract and instructions to distinguish the selected pause duration, the provider’s maximum permitted duration, and the number of avoided billing cycles. I also added deterministic validation and regression coverage for these values. In the final evaluation, the agent correctly recommended a 57-day pause within the 60-day maximum, identified two avoided billing cycles, calculated $31.98 in temporary savings, and explained that the normal $15.99 monthly charge would return afterward.

The clearest before-and-after improvement concerned evaluation grading. Early responses sometimes failed because JavaScript searched for particular words even when the response communicated the correct meaning. The keyword grader was removed. Exact machine-checkable properties remained in deterministic code, while semantic requirements moved to the independent judge. Later false negatives involving multiple titles, action timing, bundles, pause duration, and child-rating exceptions were resolved by completing the runtime information and making the rubrics principle-based. After the memory architecture, instruction cleanup, feedback workflow, hybrid retrieval, and audience-language changes, the local release gate passed all 264 explicitly counted tests: 102 of 102 component-quality checks, 127 of 127 context-retrieval benchmark cases, 21 of 21 feedback and regression-workflow checks, 7 of 7 provider-connector contract checks, and 7 of 7 structure and publishing-boundary checks. The separate compound cross-component gate also passed across all ten official evaluation information packets, all 40 positive retrieval fixtures, and the additional workflow and integration assertions. The official final live run on August 8, 2026 used instruction-bundle fingerprint `a96272d6` and exported evaluation-configuration fingerprint `966b686e`, and passed all ten evaluation cases with zero validation failures, zero API errors, zero material judge gaps, and preserved human-control and safety boundaries. Because this run followed the later feedback, free-text routing, preference-approval, regression-capture, hybrid-retrieval, and audience-language changes, it closes the previous evaluation-evidence freshness gap.
...
- Knowledge, tools, hybrid context retrieval, and persistent memory are implemented in browser JavaScript and localStorage. Context retrieval combines exact keyword, fuzzy, local semantic-concept, and household-relationship matching over bundled fictional records, with structured coverage validation and portfolio expansion. It is not a production embedding service, vector database, knowledge graph, external retrieval platform, or production database.
...
7. A third Enter a manual scenario path demonstrates that the same agent can interpret any nonsensitive free-text message through the selected LLM, answer subscription-related questions, politely redirect out-of-scope requests, record adult-confirmed subscription, plan, budget, watchlist, and viewing changes, collect recommendation feedback, and propose lasting preferences for explicit adult review.
...
10. Finally, the Evals tab shows the ten-case evaluation suite covering missing-data abstention, bundle reasoning, no-action restraint, billing and legal escalation, Subscribe, Pause, and child-rating safety. The official final live run completed on August 8, 2026 under instruction-bundle fingerprint `a96272d6` and exported evaluation-configuration fingerprint `966b686e`, after the latest instruction and model-input changes. All ten cases passed with zero validation failures, zero API errors, zero material judge gaps, and preserved human-control and safety boundaries.
