...
Testing is divided into deterministic and probabilistic layers. The deterministic release gate requires 100% passing results and currently covers 93 component assertions, 21 feedback and regression-capture checks, 126 hybrid-retrieval cases, all ten official evaluation information packets, and 40 positive retrieval fixtures. The live evaluation layer tests the selected agent and judge models against the ten fixed cases and requires at least a 95% pass rate across repeated runs.
...
The deterministic release gate passed 93 of 93 component assertions, 21 of 21 feedback and regression-capture checks, and 126 of 126 hybrid-retrieval cases. Cross-component regression confirmed that all ten official evaluation information packets retained their required services, titles, viewers, records, terms, URLs, rules, and deterministic calculations, and all 40 positive retrieval fixtures passed. The generated household-information bundles also passed the audience-language checks that prevent filenames, technical selection metadata, schemas, hashes, and raw internal identifiers from being presented as household information to the LLM.
The latest completed live evaluation baseline was run on July 29, 2026 at 12:01 PM PDT under prompt hash `05971ddc`. Nine model-driven cases used `gpt-5.6-terra` for agent responses and `gpt-5.6-luna` for independent semantic judging. EVAL-07 used the shared deterministic signal detector and made no model call. All ten cases passed with zero validation failures, zero API errors, zero material judge gaps, and preserved human-control and safety boundaries. This run predates the subsequent feedback workflow, nonsensitive free-text routing, lasting-preference approval, regression-draft capture, and audience-language changes, so it is retained as the latest completed historical live baseline rather than labeled as the final current run. A fresh ten-case live evaluation must be completed before recording a new current prompt hash and final live evidence.
...
10. Finally, the Evals tab shows the ten-case evaluation suite covering missing-data abstention, bundle reasoning, no-action restraint, billing and legal escalation, Subscribe, Pause, and child-rating safety. The latest completed historical live baseline shows all ten cases passing under prompt hash `05971ddc`, with zero failures, zero API errors, and zero material judge gaps. Because the feedback workflow, free-text routing, lasting-preference approval, regression-draft capture, and audience-language boundary changed afterward, the dashboard must be rerun before a new result is described as the final current evaluation.
