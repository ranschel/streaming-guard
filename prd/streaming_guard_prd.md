...
The agent reads the current date; the household’s fictional prototype profile, watchlists, manually reported viewing status, priorities, preferences, active subscriptions, prices, billing dates, monthly budget, and family content rules; and the relevant conversation context. It also reads fictional service-plan and catalog records covering releases, availability changes, migrations, promotions, bundles, pause limits, cancellation terms, and account URLs. Before calling the LLM, deterministic hybrid context-selection code analyzes the current trigger or adult message, recent conversation, active recommendation, and recognized services, plans, titles, and household members. It combines exact keyword matching, typo-tolerant fuzzy matching, local semantic concepts, and household relationships to select the relevant subscription, watchlist, viewing, catalog, service-term, financial, and policy records. It validates the resulting ContextPlan for required coverage before the request is sent. Broad portfolio questions intentionally receive broader household context, while narrow questions receive a smaller relevant packet.
...
- **selected_context [dynamic context]:** Before each LLM call, the prototype creates and validates a request-specific ContextPlan. Hybrid retrieval combines exact keyword matching, typo-tolerant fuzzy matching, local semantic concepts, and household relationships among services, plans, titles, viewers, subscriptions, watchlists, viewing records, releases, ratings, rules, and calculations. The ContextPlan records the inferred intent and scope, resolved entities, required record types, selected-record counts, selection reasons, coverage status, context budget, and a stable context hash. Broad inventory, comparison, or planning questions expand to the relevant portfolio-level context, while domain gating prevents unrelated requests from retrieving household information. Missing coverage and ambiguous entities remain explicit rather than being hidden by the prompt.
...
- **send_chat_response [implemented local-output tool]:** Renders structured text, recommendations, choices, confirmations, four-section refusals, and safety responses in the active WhatsApp-style chat interface. Every adult message remains visible in the current chat exactly as entered. In-scope messages may be saved in browser-local chat history. Recognizable sensitive information and clearly out-of-scope messages remain visible only for the current session and are excluded from persistent chat history, household context, selected model context, AI request logs, and model requests. The tool does not connect to WhatsApp or another external messaging platform.
...
Transient session state is stored separately and includes the active scenario, current recommendation and discussion state, persistent in-scope chat history, reminders, workflow history, simulated-tool audit, and execution traces. Evaluation results and model connection settings are also stored separately from durable household memory. Provider API keys are never included in household context or household-data exports.
...
Every adult message remains visible in the current chat exactly as entered. Recognizable credentials, payment information, and clearly out-of-scope messages are intercepted locally before any model request. They remain visible only in current session memory and are excluded from persistent chat history, household context, selected model context, AI request logs, and model requests; the interface does not replace or mask the visible adult message. In-scope messages may enter persistent browser-local chat history after safety classification. Recognizable sensitive information found in older saved state is removed when that state loads. A model-call or classification failure does not replace or mask the adult’s visible message, and safety-only interactions cannot update household context.

### 7. Output format

*What should the agent produce so a human can review it quickly and confidently?*

For model-driven recommendations and conversations, the LLM returns strict structured JSON that follows the applicable recommendation or conversation schema. The browser validates that output and renders it as a human-readable response in the WhatsApp-style chat interface. The local no-action workflow does not call an LLM. The prototype does not generate or send email. Raw JSON and internal request details are not shown in the rendered chat, but they are available to the prototype reviewer through the explicit Copy AI log debugging export.
...
The agent never sends, stores persistently, or uses passwords, payment-card details, bank information, authentication codes, API keys, or other account credentials. Recognizable sensitive information is intercepted locally before a model request. The adult’s original message remains visible exactly as entered for the current session, but it is excluded from persistent chat history, household context, selected model context, AI request logs, and model requests. Recognizable sensitive information found in older saved state is removed when that state loads. The agent warns the adult not to share sensitive information and directs them to complete sensitive account activity through the streaming service’s official interface.

#### Out-of-scope requests

If a message is clearly unrelated to household streaming-subscription planning, management, viewing access, or spending, the prototype does not answer or perform the request. It politely explains its scope and invites a relevant question. The adult’s original message remains visible exactly as entered for the current session, but it is not sent to a model or added to persistent chat history, household context, selected model context, or AI request logs. The message cannot change household context.
...
A separate automated release gate tests the deterministic application components independently of the live LLM evaluations. It currently contains 89 component assertions covering memory, controlled writes, persistence, financial calculations, safety and validation, workflow behavior, provider configuration, and UI contracts. A 126-case hybrid-context benchmark covers keyword, semantic, mixed, negative, held-out, and portfolio-comparison queries. Cross-component regression also verifies all ten official evaluation context packets and 40 positive context-retrieval fixtures. Deterministic gates must pass at 100%; repeated model-driven evaluations must achieve at least a 95% pass rate.
...
Testing is divided into deterministic and probabilistic layers. The deterministic release gate requires 100% passing results and currently covers 89 component assertions, 126 hybrid-context retrieval cases, all ten official evaluation context packets, and 40 positive retrieval fixtures. The live evaluation layer tests the selected agent and judge models against the ten fixed cases and requires at least a 95% pass rate across repeated runs.
...
1. Added a safety-gated chat-storage workflow that keeps every adult message visible exactly as entered in the current session while preventing recognizable sensitive information and clearly out-of-scope messages from entering persistent chat history, household context, selected model context, AI request logs, or model requests. Safety-only interactions cannot update household context, and recognizable credentials in older saved state are removed during migration.
1. Added validated hybrid context retrieval combining exact keywords, typo-tolerant fuzzy matching, local semantic concepts, and household relationships. Before each LLM call, the prototype creates a structured ContextPlan containing the inferred intent and scope, resolved services, plans, titles and viewers, required record types, selected-record counts, coverage status, context budget, selection reasons, and a stable context hash.
