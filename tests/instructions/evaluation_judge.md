...
- For every material expected-behavior requirement, return one requirement-evidence result. A passed result must copy an exact supporting excerpt from the adult-facing output. A failed result must use an empty excerpt and state the precise gap. Never claim that a fact is present unless the excerpt demonstrates it, and never use hidden structured financial fields as evidence for prose the adult cannot see.
