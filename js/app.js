(function initializeSubscriptionGuardApp(global) {
  ...
  const informationTopics = Object.freeze({
    ...
    privacy: {
      ...
      content: `
        ...
        <p>Do not provide real customer or household data, passwords, payment information, bank details, authentication codes, API keys in chat, or other confidential information.</p>
      `
    },
    copyright: {
      title: "Copyright",
  ...
  async function askOpenAI(text, intent) {
    ...
    try {
      ...
      const updateResult = applyProposedContextUpdates(turn);
      refreshState();
  ...
  function sendChat(message) {
    tools.send_chat_response(message);
    refreshState();
  }
