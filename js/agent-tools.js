(function initializeAgentTools(global) {
  ...
  function createAgentTools({ memory, knowledge, clock = () => new Date().toISOString() }) {
    ...
    function sendEmailNotification({ subject, html, category = "recommendation" } = {}) {
      if (!subject || !html) throw new TypeError("subject and html are required.");
      const notification = {
        id: `email-${Date.now()}`,
        subject,
        html,
        category,
        delivery: "prototype_outbox",
        createdAt: clock()
      };
      memory.transact(state => {
        state.emailOutbox.push(notification);
      });
      recordAudit("send_email_notification", "success", category);
      return clone(notification);
    }

    function sendChatResponse({ text = "", kind = "text", role = "agent", refusalSections = null } = {}) {
